...
public class MockServiceTest {
    ...
    @Test
    public void 어떠한챔피언이름을입력해무조건베인리턴테스트(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void 어떠한포지션을세팅해도미드리턴테스트(){
        Champion champion = mock(Champion.class);
        champion.setPosition("하이");
    ...
    @Test
    public void shouldTwoTimeRunForChampionArraySize(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void shouldThreeTimeRunForChampionArraySize(){
        Champion champion = mock(Champion.class);
    ...
    @Test
    public void shouldPassNotSetName(){
        Champion champion = mock(Champion.class);
    ...
    public void List객체사이즈검증테스트(){
        ...
        champions.add(new Champion("리산드라","미드",4));
        champions.add(new Champion("티모","탑",3));
        champions.add(new Champion("초이","정글",3));
    ...
    @Test
    public void 객체리턴후스킨개수확인(){
        when(mockService.findByName(anyString())).thenReturn(new Champion("티모","탑",2));
        assertThat(mockService.findByName("아무거나").getHasSkinCount(),is(2));
        verify(mockRepository,times(1)).findByName(anyString());
    }
    // 4. 2개 이상의 가짜 챔피언 객체를 List로 만들어 리턴하고, 하나씩 해당 객체를 검색한 뒤 검색을 위해 호출한 횟수를 검증하세요.
    @Test
    public void 검색위해호출한횟수검증(){
        List<Champion> champions = new ArrayList<Champion>();
